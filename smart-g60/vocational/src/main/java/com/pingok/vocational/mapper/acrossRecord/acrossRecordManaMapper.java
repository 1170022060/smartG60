...
public interface acrossRecordManaMapper {
    @Select({"<script>" +
            "SELECT \"vehPlate\",to_char(\"enTime\", 'yyyy-mm-dd hh24:mi:ss') as \"enTime\",\"name\",\"passId\" FROM ( " +
            "SELECT SUBSTR(en.VEHICLE_ID, 1, INSTR(en.VEHICLE_ID, '_')-1) as \"vehPlate\",en.EN_TIME as \"enTime\",b.STATION_NAME as \"name\",en.PASS_ID as \"passId\" " +
            ...
            "</when>"+
            "UNION ALL " +
            "SELECT SUBSTR(mj.VEHICLE_PLATE, 1, INSTR(mj.VEHICLE_PLATE, '_')-1) as \"vehPlate\",mj.TRANS_TIME as \"enTime\",g.DEVICE_NAME as \"name\",mj.PASS_ID as \"passId\" " +
    ...
    @Select({"SELECT to_char(\"time\", 'yyyy-mm-dd hh24:mi:ss') as \"time\",\"name\" " +
            "FROM( " +
            "SELECT  " +
            "EX_TIME as \"time\", " +
            "EX_TOLL_STATION_NAME as \"name\" " +
            "FROM TBL_SHAR_ETCTD_RES_SENDER_${year} " +
            "WHERE PASS_ID =#{passId} " +
            "UNION ALL " +
            "SELECT  " +
            "EX_TIME as \"time\", " +
            "EX_TOLL_STATION_NAME as \"name\" " +
            "FROM TBL_SHAR_OTD_RES_SENDER_${year}  " +
            "WHERE PASS_ID =#{passId} " +
            "UNION ALL " +
            "SELECT  " +
            "TRANS_TIME as \"time\", " +
